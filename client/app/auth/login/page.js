        ...
        <div className="bg-white border border-[#e8ede9] rounded-3xl p-8 flex flex-col gap-6">
          <div className="text-center">
            ...
            <p className="text-[14px] text-[#3d4f45] font-medium leading-relaxed">
              Sign in to your Roppal account
            </p>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-white border-[1.5px] border-[#d4ddd6] hover:border-[#a8d4bc] hover:bg-[#f0f4f1] text-[#0a0f0c] font-semibold text-[14px] rounded-2xl px-5 py-3.5 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {loading ? (
              <>
                <svg className="w-5 h-5 animate-spin text-[#1f6b45]" fill="none" viewBox="0 0 24 24">
          ...
          <div className="flex flex-col gap-3">
            {[
              ...
              ["⚡", "Sign in in one tap — instant access"],
              ["🛡️", "Secured by Google's authentication"],
            ].map(([icon, text], i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-xl bg-[#e8f5ee] flex items-center justify-center text-[15px] flex-shrink-0">
                  {icon}
                </div>
                <p className="text-[13px] text-[#3d4f45] font-medium">{text}</p>
              </div>
            ))}
          </div>
          ...
          <p className="text-center text-[13px] text-[#3d4f45] font-medium">
            ...
            <a href="/auth/register" className="text-[#1f6b45] font-semibold hover:underline">
              Sign up
            </a>
          </p>
        </div>
